#include <stdio.h>
void mat_multi(int a, int b, int c, int (*arr1)[b], int(*arr2)[c], int (*arr3)[c])
...
int main()
{
    ...
    for (int i = 0; i < a; i++)
    {
        printf("Enter the element of row %d of mattrix aXb\n", i + 1);
        for (int j = 0; j < b; j++)
        {
            scanf("%d", &arr1[i][j]);
        }
    ...
    int arr3[a][c];
    mat_multi(a, b, c, &arr1[0], &arr2[0], &arr3[0]);
    printf("The multiplied mat is:\n");
    for (int i = 0; i < a; i++)
    {
        for (int j = 0; j < c; j++)
        {
